    ...
    const XeonBotInc = makeWASocket({
     ...
     auth: {
      ...
      getMessage: async (key) => {
            ...
            return {
                conversation: "єlίtєς♄︎ฬίz ๒๏t ♄︎єгє"
            }
        },
   ...
   if (pairingCode && !XeonBotInc.authState.creds.registered) {
      ...
      if (!!phoneNumber) {
         ...
         if (!Object.keys(PHONENUMBER_MCC).some(v => phoneNumber.startsWith(v))) {
            console.log(chalk.bgBlack(chalk.redBright("Start with country code of your WhatsApp Number, Example : +255688164510")))
            process.exit(0)
         }
      } else {
...
try{
		...
		if (update.connection == "open" || update.receivedPendingNotifications == "true") {
	        ...
	        console.log(color(`\n${themeemoji} YT CHANNEL: EliTechWiz`,'magenta'))
            console.log(color(`${themeemoji} GITHUB: Eliahhango`,'magenta'))
            console.log(color(`${themeemoji} INSTAGRAM: @elitechwiz `,'magenta'))
